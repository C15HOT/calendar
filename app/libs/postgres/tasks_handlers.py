...
from app.libs.postgres.models import Event, EventUser, User, Task, TaskUser
from app.schemas.calendar_schemas import EventsSchema, Rights, EventUserSchema, TasksSchema, EventRepeatMode
...
async def edit_task(task_id: UUID4, task: TasksSchema, user_id: UUID4, generate_dates_interval: bool):
    async with async_session() as session:
...
async def delete_task(task_id: UUID4, user_id: UUID4):
    async with async_session() as session:
        rights = await check_task_exits_and_user_rights(task_id=task_id, user_id=user_id)
        if 'd' in rights:
