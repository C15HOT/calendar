...
from app.libs.ml_app.libs.bd import get_time_segment_user_events, get_enveloping_user_events
from app.libs.ml_app.libs.geography import GeoStuff
...
async def check_intersections(
    event: EventsSchema,
    user_id
) -> List[EventsSchema]:  # список событий, накладывающихся на переданное в аргументах событие

    overlapping_events_cursor_bin = await get_time_segment_user_events(
        user_id=user_id,
        before_ts=event.from_datetime,
...
async def generate_event_variants(user_id, duration, location_begin, location_end):
    ...
    if len(future_events) == 0:
        ...
        considered_start = only_event.to_datetime + timedelta(days=1)
        while not 9 <= considered_start.hour <= 18:
            considered_start += timedelta(hours=1)
        weekend_delta = 0 if considered_start.weekday() < 5 else 7 - considered_start.weekday()
        considered_start += timedelta(days=weekend_delta)
        ...
        if future_events[0].from_datetime - considered_start >= duration + road_to_first_time:
            all_time_slots.add(considered_start)
        for current_event_index in range(0, len(future_events) - 1):
            ...
            current_finish = current_event.to_datetime
            next_begin = next_event.from_datetime
    ...
    last_event_end = max(last_event.to_datetime if last_event else considered_start + timedelta(hours=2),
                         considered_start)
...
async def get_cancellations(
    ...
    if other_events_importance > current_event_importance:
        cancelled_event_duration = event.to_datetime - event.from_datetime
